  ...
  return (
    <div className="container py-12">
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-10">
        <Stat label={t("seller.publications")} value={submissions.length} />
        <Stat label={t("seller.approved")} value={submissions.filter((s) => s.approved).length} />
        <Stat label={t("seller.pendingCount")} value={submissions.filter((s) => !s.approved).length} />
      </div>

      <h2 className="font-display text-2xl text-brown-ink mb-4">{t("seller.myPublications")}</h2>
      {submissions.length === 0 ? (
        <p className="text-muted-foreground italic">{t("seller.nothingPublished")}</p>
      ) : (
              ...
              <div className="text-right">
                {s.approved ? (
                  ...
                  <span className="inline-flex items-center gap-1 text-xs text-mustard-deep font-semibold uppercase">
                    <Clock className="h-3.5 w-3.5" /> {t("seller.inReview")}
                  </span>
                )}
